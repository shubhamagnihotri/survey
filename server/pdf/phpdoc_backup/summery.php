...
//
	...
	$sql_state = "select distinct task_state, state_name from nlp_tasks, nlp_state where state_id=task_state and task_nlcpr_project=1 and task_state != 82 order by state_id";
	...
	$Tcolsp=$colsp+1;
	 while($row_state = mysql_fetch_array($log_state))
     {
		...
		$sta_id = $row_state[0];
		$sql_check = "select distinct task_id from nlp_tasks where task_state=$sta_id and task_nlcpr_project=1";
		$tasklist="";
		$log_check = mysql_query( $sql_check);
		
		while($row_check = mysql_fetch_array($log_check)){
			$tasklist_id = $row_check[0];
			$tasklist =  $tasklist_id. ",".$tasklist ;
			if(!$row_check){ break; }
		}
		
		$tasklist = $tasklist."0";
		$sql_task =  "select sum(task_target_budget) as task_target_budget, sum(task_no_of_nlcpr_project) as No_of_project from nlp_tasks where task_state=$sta_id and task_nlcpr_project=1";
		...
		while($ii<$colsp1-1)
		 {
			...
			$sql_sanction11[$ii] = "select sum(sanction_dept_amount) as rel, sum(sanction_loan_amount) as rel1 from nlp_sanction where sanction_state = $sta_id and sanction_first_date between '$year33[$ii]' and '$year44[$ii]' and sanction_task in ($tasklist) order by sanction_first_date";

			
			$log_sanction11[$ii] = mysql_query($sql_sanction11[$ii]);
			$row_sanction11[$ii] = mysql_fetch_array($log_sanction11[$ii]);
			$yearlyTotal3 = ($row_sanction11[$ii][0]+$row_sanction11[$ii][1])/100;
		...
		while($i < $colsp-1){
			...
			$year2[$i] = str_pad($yearfrom+1,10,"/03/31");
			 ...
			 $sql_sanction[$i] = "select sum(sanction_dept_amount) as rel, sum(sanction_loan_amount) as rel1 from nlp_sanction where sanction_state = $sta_id and sanction_first_date between '$year1[$i]' and '$year2[$i]' and sanction_task in ($tasklist) order by sanction_first_date";

			
			$log_sanction[$i] = mysql_query($sql_sanction[$i]);
			$row_sanction[$i] = mysql_fetch_array($log_sanction[$i]);
			$yearlyTotal = ($row_sanction[$i][0]+$row_sanction[$i][1])/100;
		...
		while($ii2<$colsp2-1)
		 {
			...
			$sql_sanction1[$ii2] = "select sum(sanction_dept_amount) as rel, sum(sanction_loan_amount) as rel1 from nlp_sanction where sanction_state = $sta_id and sanction_first_date between '$year3[$ii2]' and '$year4[$ii2]' and sanction_task in ($tasklist) order by sanction_first_date";

			
			$log_sanction1[$ii2] = mysql_query($sql_sanction1[$ii2]);
			$row_sanction1[$ii2] = mysql_fetch_array($log_sanction1[$ii2]);
			$yearlyTotal1 = ($row_sanction1[$ii2][0]+$row_sanction1[$ii2][1])/100;
