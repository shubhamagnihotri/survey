...
$to = substr($year_to, 8, 2)."/".substr($year_to, 5, 2)."/".substr($year_to, 0, 4);
	...
	$sql_state = "select distinct task_state, state_name from nlp_tasks, nlp_state where state_id=task_state and task_nlcpr_project=1 order by state_id";
...
$sector_html.='<table border="0" cellspacing="1" cellpadding="1" width="100%">';
	...
	$sql_proj="select project_id, project_name from ".$dbprefix."projects order by project_name";
	$log_proj = mysql_query( $sql_proj );

	while($row_proj = mysql_fetch_array($log_proj)) {
		$colsp= $colsp + 1;
     	$proj_id[$colsp] = $row_proj[0];
     	$proj_name[$colsp] = $row_proj[1];
	...
	<tr cellspacing="3" bgcolor="#9BDE9B">
		...
		<td rowspan="2" align="left" >'.$row_state["state_name"].'</td>';
			...
			$sql_task = "select sum(task_target_budget) as task_target_budget, sum(task_no_of_nlcpr_project)  as No_of_project
			            from ".$dbprefix."tasks where task_state=$sta_id and task_project = $proj_id[$i] and
						task_start_date between '$year_from' and '$year_to' and task_nlcpr_project=1";

			$log_task = mysql_query( $sql_task);
			$row_task = mysql_fetch_array($log_task);

			$total_project[$i] = $row_task[1];
			$cost_project[$i] = $row_task[0];
