...
$n = 0;	
  ...
  <tr bgcolor="#D1CC00">
   ...
   while($row_project = mysql_fetch_array($log_project)){
		...
		$sql_utilisation = "select utilisation_amount as util from ".$dbprefix."utilisation where utilisation_date <= '$today' and utilisation_task = $task_id"; 
		...
		if ($flag=="y") {
			...
			<td colspan="2" >
				<table>';
					$sql_uc = "select utilisation_date, utilisation_amount from ".$dbprefix."utilisation where utilisation_date <='$today' and utilisation_task = $task_id order by utilisation_date desc";
