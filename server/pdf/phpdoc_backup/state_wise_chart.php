...
$sql_project = "SELECT s.state_name, SUM(t.task_no_of_nlcpr_project) AS No_of_project ,(SELECT  SUM(task_no_of_nlcpr_project) AS No_of_project FROM nlp_tasks WHERE task_state=s.state_id AND task_project IN (SELECT project_id FROM nlp_projects) AND task_start_date BETWEEN '1998/04/01' AND REPLACE(CURDATE(),'-','/') AND (task_actual_finish_date IS NOT NULL AND task_actual_finish_date != '0000-00-00') AND task_nlcpr_project=1
) AS completed_projects FROM nlp_tasks AS t , nlp_state AS s WHERE  t.task_project IN (SELECT project_id FROM nlp_projects)  AND task_start_date BETWEEN '1998/04/01' AND REPLACE(CURDATE(),'-','/') AND t.task_nlcpr_project=1 AND t.task_state=s.state_id  GROUP BY t.task_state order by s.state_name asc";
