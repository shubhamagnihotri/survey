...
$q->addQuery("p.project_name, SUM(t.task_no_of_nlcpr_project) AS No_of_project ,(SELECT  SUM(task_no_of_nlcpr_project) AS No_of_project FROM nlp_tasks WHERE task_state IN (SELECT state_id FROM nlp_state) AND task_project = p.project_id AND task_start_date BETWEEN '1998/04/01' AND REPLACE(CURDATE(),'-','/') AND (task_actual_finish_date IS NOT NULL AND task_actual_finish_date != '0000-00-00') AND task_nlcpr_project=1
) AS completed");
$q->addWhere("t.task_state IN (SELECT state_id FROM nlp_state)  AND task_start_date BETWEEN '1998/04/01' AND REPLACE(CURDATE(),'-','/') AND t.task_nlcpr_project=1 AND t.task_project=p.project_id");
