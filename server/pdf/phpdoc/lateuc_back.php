...
foreach($arr_state as $state_row){
...
$q->addTable("tasks");
$q->addTable("sanction");
$q->addQuery("distinct(task_id), task_project, max(sanction_first_date)");
$q->addWhere("sanction_first_date <= '$today' and sanction_task=task_id and (task_actual_finish_date = '0000-00-00' or task_actual_finish_date is null) and task_start_date between '$year_from' and '$year_to' and task_state=$state_id");
$q->addGroup("task_id");
$q->addOrder("task_project, task_id");
...
$q->addTable("sanction");
            ...
            $q->addWhere("sanction_first_date <= '$today' and sanction_task = $task_id");
            $log_sanction = $q->loadList();
            $q->clear();
            
		
		foreach ($log_sanction as $row_sanction) {
			$total_rel = $total_rel + ($row_sanction['rel']+$row_sanction['rel1']);
		}
 $q->addTable("utilisation");
            $q->addQuery("sum(utilisation_amount) as util");
            $q->addWhere("utilisation_date <= '$today' and utilisation_task = $task_id and utilisation_status = 1");
            $log_utilisation = $q->loadList();
            $q->clear();
	
	
		foreach ($log_utilisation as $row_utilisation ) {
		...
		if (($total_rel != $total_uc) && ($diff_amount >= (0.25 * $lastrel)) && ($diff_date >= 12)) {
			 $sql_task =  "select task_name,task_start_date, task_target_budget,task_actual_finish_date, task_no_of_nlcpr_project, task_id from ".$dbprefix."tasks where task_id=$task_id and task_start_date between '$year_from' and '$year_to' and task_state=$state_id and task_nlcpr_project=1";
