...
$q->addTable("tasks", "t");
$q->addTable("state", "s");
$q->addQuery("s.state_name ,SUM(task_no_of_nlcpr_project) AS No_of_project,SUM(task_target_budget) AS task_target_budget ,(SELECT  (SUM(sanction_dept_amount) + SUM( sanction_loan_amount))  FROM nlp_sanction, nlp_tasks, nlp_state WHERE sanction_state=state_id  AND sanction_task =task_id
...
(SELECT SUM(utilisation_amount) FROM nlp_utilisation, nlp_state WHERE utilisation_date <=REPLACE(CURDATE(),'-','/') AND utilisation_status = 1 AND utilisation_task IN (SELECT task_id FROM nlp_tasks WHERE utilisation_date BETWEEN '1998/04/01' AND REPLACE(CURDATE(),'-','/') AND  task_state=state_id  AND task_nlcpr_project=1)   
AND state_id=s.state_id) AS uc");
$q->addWhere("task_state=s.state_id  AND task_start_date BETWEEN '1998/04/01' AND REPLACE(CURDATE(),'-','/') AND task_nlcpr_project=1 ");
$q->addGroup("s.state_id");
